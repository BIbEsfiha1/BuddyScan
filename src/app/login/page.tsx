...
export default function LoginPage() {
  ...
  // Determine if there's a critical initialization error (auth instance unavailable)
  const isAuthUnavailable = !auth || !!contextAuthError; // Check if auth object is null/undefined or if context has error
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<LoginFormInputs>({
   ...
   useEffect(() => {
       // Ensure auth is available before checking user
       if (isAuthEnabled && !authLoading && user && auth) {
   ...
   const handleAuthError = (error: any, providerName: string) => {
        ...
        // Basic error code handling (can be expanded)
        if (error.code) {
            switch (error.code) {
                ...
                case 'auth/argument-error':
                    ...
                    console.error("Auth Argument Error - Check Firebase config (authDomain, API Key restrictions, authorized domains in Firebase Console).");
                    break;
  ...
  const onEmailSubmit = async (data: LoginFormInputs) => {
     ...
     try {
       ...
       // Auth instance is checked via isAuthUnavailable
       ...
       if (!isAuthEnabled || isAuthUnavailable) {
        ...
        switch (providerType) {
            case 'google':
                ...
                providerName = 'Google';
                break;
            // Keep other cases disabled for now
            case 'facebook':
            case 'twitter':
        ...
        try {
            ...
            // Auth instance is checked via isAuthUnavailable
            console.log("[DEBUG] Auth Instance Config (Login):", auth!.config); // Log config before call

            // Check authDomain specifically before the call
      ...
      <Card className="w-full max-w-md shadow-xl border-primary/20 card">
        <CardHeader className="text-center">
             <Image
                  ...
                  // Removed onError handler to avoid console noise if path is correct
              />
        ...
        <CardContent className="space-y-6">
            {/* Show critical auth error from context if present */}
           ...
           {/* Login Form */}
            <form onSubmit={handleSubmit(onEmailSubmit)} className="space-y-4">
                ...
                {/* Display login-specific errors (not critical init errors) */}
                ...
                <Button type="submit" className="w-full font-semibold button" disabled={isLoading || isAuthUnavailable || !!socialLoginLoading || !isAuthEnabled}>
                 ...
                 {/* Add message if auth is generally disabled by flag */}
                 {!isAuthEnabled && (
