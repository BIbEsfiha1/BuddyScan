...
export default function LoginPage() {
  ...
  // Determine if there's a critical initialization error (Firebase failed to init)
  // Use contextAuthError provided by useAuth which reflects errors during initialization or listener setup
  const isAuthUnavailable = !!contextAuthError && !authLoading; // Consider unavailable if there's an error *after* loading finishes
  ...
  // --- Redirect Effect ---
  // Redirect logged-in users away from the login page *after* auth state is confirmed
  useEffect(() => {
    // Only redirect if auth is NOT loading AND a user object exists
    if (!authLoading && user) {
      console.log("User logged in (useEffect check on login page), redirecting to /dashboard...");
  ...
  const handleAuthError = (error: any, providerName: string) => {
    ...
    if (error.code) {
      switch (error.code) {
        ...
        case 'auth/invalid-credential': // General incorrect credentials (can include wrong password)
        case 'auth/wrong-password': // More specific, but invalid-credential covers it
  ...
  const onEmailSubmit = async (data: LoginFormInputs) => {
    // Check if Firebase auth service is available (auth instance exists)
  ...
  const handleSocialLogin = async (providerType: 'google' | 'facebook' | 'twitter') => {
    ...
    try {
       ...
       console.log(" auth.config:", auth.config); // Log the actual config used by the live auth instance
   ...
   if (authLoading || (!authLoading && user)) { // Show loading if auth is loading OR if user exists (implies redirect is pending)
       return (
   ...
   // Render the login form only if auth has finished loading and there is no user
  return (
    <TooltipProvider>
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-background via-muted/50 to-primary/10">
          ...
          <CardContent className="space-y-6">
            ...
            {isAuthUnavailable && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  {contextAuthError?.message || 'Serviço de autenticação indisponível.'} A autenticação pode não funcionar. Verifique as configurações ou contate o suporte.
                </AlertDescription>
              </Alert>
            )}
